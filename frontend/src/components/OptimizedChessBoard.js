...
const OptimizedChessBoard = ({
  ...
  const actualFen = (fen && typeof fen === 'string' && fen.trim() !== '') 
    ? fen 
  ...
  useEffect(() => {
    ...
    if (newFen && typeof newFen === 'string' && newFen.trim() !== '' && newFen !== currentFen) {
      try {
        ...
        console.error("Failed to load FEN in OptimizedChessBoard:", newFen, e);
        ...
        gameRef.current.load(fallbackFen);
      }
    }
  ...
  const handlePieceDrop = useCallback((sourceSquare, targetSquare) => {
    ...
    try {
      ...
      console.error("Illegal move in OptimizedChessBoard:", e);
    }
    return false;
  ...
  return (
    ...
    >
      ...
      >
        {/* Custom board implementation to prevent flashing */}
        <div
          className="optimized-board"
          style={{
            width: '100%',
            height: '100%',
            backgroundColor: '#f0d9b5',
...
export default React.memo(OptimizedChessBoard);
