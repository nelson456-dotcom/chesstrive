...
const UserStats = ({ stats: propStats, userRating, blunderRating, visualisationRating, endgameRating, advantageRating, advantageWins, defenderWins, openingsPracticed: propOpenings = 0, puzzleRushBestStreak = 0 }) => {
  const { user } = useAuth();
  ...
  const openingsPracticed = (stats?.openingStats?.openings?.length || 0) || propOpenings || user?.repertoire?.length || 0;
  ...
  return (
    <div className="bg-white shadow-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 border border-gray-100">
      ...
      {/* Advantage Conversion and Defender Wins */}
      <div className="mb-6 sm:mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 sm:p-6 rounded-xl border border-blue-200 shadow-lg">
            <div className="flex items-center justify-between mb-2 sm:mb-3">
              <h4 className="text-base sm:text-lg font-semibold text-blue-900">Defender Wins</h4>
              <div className="w-6 h-6 sm:w-8 sm:h-8 bg-blue-500 rounded-full flex items-center justify-center">
            ...
            <p className="text-xs sm:text-sm text-blue-700">Positions won</p>
          </div>
        </div>
      </div>
