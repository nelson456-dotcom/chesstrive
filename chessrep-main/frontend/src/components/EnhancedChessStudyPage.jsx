...
const EnhancedChessStudyPage = () => {
  ...
  const navigateToPosition = useCallback((path, moveIndex) => {
    ...
    movesToPlay.forEach(move => {
      ...
      if (!notation) {
        console.warn('⚠️ navigateToPosition: move missing notation', move);
        return;
      }
      ...
      try {
        ...
        if (!moveResult) {
          console.warn('⚠️ navigateToPosition: could not apply move (received null result):', notation, move, 'Current FEN before move:', newGame.fen());
          return;
        }
      } catch (error) {
        console.error('❌ Failed to apply move during navigation:', notation, error, move);
      }
    });
  ...
  const loadChapterState = async (chapterId) => {
    ...
    try {
      ...
      if (chapter.gameTree && chapter.gameTree.moves && chapter.gameTree.moves.length > 0) {
        ...
        console.log('🎯 Setting board to chapter starting position:', startingFen);
