...
// Reusable button to open the current position in the Chess Analysis Board
// Props:
// - fen: string (required)
...
const OpenInLiveAnalysisButton = ({
  ...
  const href = useMemo(() => {
    ...
    return `/chess-analysis-board${search ? `?${search}` : ''}`;
  ...
  return (
    <a
      ...
      title="Open this position in the Chess Analysis Board"
    >
