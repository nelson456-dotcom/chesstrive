...
const InteractiveMenu: React.FC<InteractiveMenuProps> = ({ items, accentColor, onItemClick, initialActiveIndex = 0 }) => {
  ...
  useEffect(() => {
    if (initialActiveIndex !== undefined && initialActiveIndex >= 0 && initialActiveIndex < finalItems.length) {
      setActiveIndex(initialActiveIndex);
    }
  }, [initialActiveIndex, finalItems.length]);
  useEffect(() => {
  ...
  const handleItemClick = (index: number) => {
    ...
    if (onItemClick && finalItems[index]) {
