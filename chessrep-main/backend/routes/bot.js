...
async function getStockfishMove(fen, engineParams, personality) {
  return new Promise((resolve, reject) => {
    ...
    // Ensure minimum move time for quality (at least 1 second, more for higher levels)
    const movetime = Math.max(1000, Math.round(moveTimeMs || 2000));
    ...
    // Safety timeout - give Stockfish plenty of time (at least 2x movetime + 5 seconds buffer)
    // For Grandmaster (10s movetime), timeout = 25 seconds minimum
    const timeout = setTimeout(
      () => finish(new Error('Engine timeout')),
      Math.max(15000, movetime * 2 + 5000)
    );
    ...
    engine.stdout.on('data', (data) => {
      ...
      if (uciOk && optionsSet && out.includes('readyok')) {
        ...
        // ALWAYS use movetime to ensure Stockfish actually thinks for the full duration
        // For full strength: only movetime (no depth/node limits)
        // For limited strength: use movetime as primary, depth/nodes as secondary limits
        const goArgs = [];
        
        if (useFullStrength) {
          // Full strength: ONLY movetime - let Stockfish search as deep as it can in the time
          console.log(`[Stockfish] Starting FULL STRENGTH search with ${movetime}ms time limit...`);
          goArgs.push(`movetime ${movetime}`);
        } else {
          // Limited strength: prioritize movetime, but also respect depth/nodes if specified
          console.log(`[Stockfish] Starting LIMITED strength search (Elo ${limitedElo}) with ${movetime}ms time limit...`);
          goArgs.push(`movetime ${movetime}`);
          // Add depth/nodes as additional constraints (Stockfish will respect both)
          if (finalDepth) {
...
router.post('/move', optionalAuth, async (req, res) => {
  try {
    ...
    // Note: Thinking time is now handled by Stockfish's movetime parameter
    // Stockfish will actually think for the configured time before returning a move
    // No need for artificial delay - the move quality comes from Stockfish's actual search time
    console.log(`✅ Bot move generated by Stockfish (rating: ${targetRating}, actual search time used)`);
